class Graphics {
  ...
  render(simulation) {
    ...
    // Pheromones
    {
      ...
      imagedataFilters.brightness(iData, {amount: this.sittings.brightness});
    ...
    // Ants
    {
      ...
      let antXPos;
      let antYPos;
      let antWidth = this.sittings.antSize[0];
      let antHeight = this.sittings.antSize[1];
      let useImage = this.sittings.useImage;

      function draw(x, y, w, h, useImage, image) {
        if (useImage) {
...
class Settings {
  constructor(antSize ,accuratePosition, enabledRotation, useImage, brightness) {
    this.accuratePosition = (accuratePosition === undefined) ? true : accuratePosition;
    this.enabledRotation = (enabledRotation === undefined) ? true : enabledRotation;
    this.useImage = (useImage === undefined) ? false : useImage;
    this.antSize = antSize || [4, 6];
    this.brightness = brightness || 1.5;
  }
}
